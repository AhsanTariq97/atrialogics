...
const ImageLightbox = ({ imageUrl }: {imageUrl: any}) => {
    ...
    const onlyImage = imageUrl.replace(/<figcaption\b[^>]*>(.*?)<\/figcaption>/i, '')
        ...
        <>
            ...
            {isOpen && (
                <div className="fixed top-0 left-0 z-50 flex items-center justify-center w-full h-full bg-black/90" >
                    <div ref={imageRef} className="relative max-w-[90%] max-h-[90%] overflow-hidden bg-transparent rounded-xl">
                        <div className={`max-w-full max-h-full cursor-zoom-in ${isZoom && 'scale-150 cursor-zoom-out'}`} dangerouslySetInnerHTML={{ __html: onlyImage }}
                        onClick={() => setIsZoom(prev => !prev)}
                        />
                    </div>
                    <button onClick={() => setIsOpen(false)} className='fixed top-4 right-4'><IoClose size={35} className='text-white/80' /></button>
                </div>
            )}
        </>
    )
